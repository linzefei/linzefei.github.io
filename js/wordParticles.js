...
class WordParticles {
        ...
        for (let y = 0; y < H; y += 3) {
            for (let x = 0; x < W; x += 3) {
                if (data[(y * W + x) * 4 + 3] > 100) {
                    pixels.push({ nx: x / W - 0.5, ny: -(y / H - 0.5) });
                }
            }
        }
        return pixels;
    ...
    _init() {
        ...
        pixels.forEach((p, i) => {
            ...
            const r = 80 + Math.random() * 500; const th = Math.random() * Math.PI * 2;
        ...
        this.mat = new THREE.PointsMaterial({ size: 2.8, vertexColors: true, transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false, sizeAttenuation: true });
    ...
    update() {
        ...
        if (this.phase === 'assembling') {
            ...
            for (let i = 0; i < this.n; i++) {
                ...
                const angle = (1 - ease) * Math.PI * 2.5;
